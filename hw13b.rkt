...
(define (compile expr)
  ...
  (lambda (env)
    ...
    ;; convient helper
    (: caller-map : (Listof TOY) -> (ENV -> VAL) -> VAL)
    (define (caller-map exprs)
      (map (lambda (x) (caller env)) exprs))
    (cases expr
    [(Num n)   (RktV n)]
   ...
   ; (set-box! (lookup name env) (compile* new))
     ...
     (set-box! (lookup name env) (caller env))



   ; (set-box! (lookup name env) ((caller env) new))
     the-bogus-value]
    [(Bind names exprs bound-body)
     (compile-body bound-body (extend names (caller-map exprs env) env))]
    ...
    [(Call fun-expr arg-exprs)
     (let ([fval (compile* fun-expr)]
           ...
           [arg-vals (lambda () (caller-map arg-exprs))])
    ...
    [(If cond-expr then-expr else-expr)
     (caller (if (cases (compile* cond-expr)
